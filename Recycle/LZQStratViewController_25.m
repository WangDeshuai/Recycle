...
- (void)addSrollView{
    ...
    NSMutableArray * starArr = [[NSMutableArray alloc]initWithObjects:@"引导页1",@"引导页2",@"引导页3",@"引导页4", nil];
...
- (void)addBtn{
    _btn = [[UIButton alloc] initWithFrame:CGRectMake(SCR_W * 3 + SCR_W / 2-75, (GAO/2-70)*GAO/667, 150, 77/2)];
    if (KUAN==320){
        _btn.frame=CGRectMake(SCR_W * 3 + SCR_W / 2-75, GAO/2-50, 150, 77/2);
    }else if (KUAN==414){
         _btn.frame=CGRectMake(SCR_W * 3 + SCR_W / 2-75, GAO/2-100, 150, 77/2);
    }
    else{
      _btn.frame=CGRectMake(SCR_W * 3 + SCR_W / 2-75, GAO/2-70, 150, 77/2);
    }
   // NSLog(@">>>%f>>>%f",KUAN,GAO);
    [_btn addTarget:self action:@selector(clickBtn) forControlEvents:UIControlEventTouchUpInside];
    [_btn setBackgroundImage:[UIImage imageNamed:@"马上进入"] forState:0];
